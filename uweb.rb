...
#
#   Specify the directory containing the ERB template files to use. By default,
#   uweb will use the template files located next to the program itself.
#
# -o file, --output file
#
...
# each command must appear in a line of its own and the order of attributes
# must be exactly as described below. However, you can be free with white
# spaces, the quote character you use and the case (upper or lower).
#
...
# indicators such as //, /* ... */, -- etc., which are ignored. In contrast,
...
# matter of modifying the regular expression used to detect chunk comments.
# Note that specifying the name after the end of the chunk is optional, but it
# if is specified it must match the name given at the beginning of the chunk.
#
...
# uweb to calculate all cross references. Hence uweb uses a <link> tag which
# must appear in the <head> section of the X/HTML document, before any of the
# <body> content.
#
# === <a rel='include' name='relative path'>
#
# Includes the content of the specified file at this point, as if it was part
# of the original input. This allows breaking up the input to several files for
# easier maintenance.
#
# === <a rel='chunk' name='chunk name'>
#
# Embed the specified named chunk at this point of the  generated
# documentation. This command may only appear after the last <link rel='chunk'>
# was processed. This should automatically be the case as <link> tags may only
# appear in the <head> section. The chunk name must match one of the chunks
# detected when scanning the linked source files.
#
# If all the chunk's text lines are indented by some amount, then it is
# stripped from the generated documentation lines. Nested chunks are converted
# to hyperlinks; they need to be explicitly embedded elsewhere in the
# documentation.
#
# == Output
#
...
#
# A common practice is to pipe the generated X/HTML through htmltoc, hypertoc
# or a similar program to automatically generate a table of contents based on
# the standard X/HTML header tags (<h1>, <h2>, ...).
#
...
# === uweb.chunk
#
# This template is used to convert the <a rel='chunk'> tag into X/HTML
# documentation. Variables accessible to ERB are:
#
...
#
# locations::  List of the locations of this chunk in the scanned source
#              files. Typically a chunk only appears in a single source file.
#              However, some chunks are "reused" across several source files.
...
#
# The key idea (which uweb shamelessly lifts from ProgDoc) is to invert the
# "tangle" step and incorporate it into the "weaving". That is, instead of
# generating the source from chunks, extract the chunks from the source files,
# and embed them into placeholders specified in the documentation. This is how
# most code-related papers or articles are written in practice.
#
...
# == Why uweb
#
# There are many literate programming tools, each with its own set of
# trade-offs. The uweb tool goals are, in descending order of importance:
#
# === Inverse literate programming
#
...
# = Author
#
# Oren Ben-Kiki (uweb-oren@ben-kiki.org).
#
# = Version
#
# This is uweb version 0.2. For the latest version see
# http://rubyforge.org/projects/uweb/.
#
# = License
#
# Copyright (C) 2008 Oren Ben-Kiki
#
...
def embed_chunk(name)
  ...
  chunk.is_used = true
  name = chunk.name
  refers_to = chunk.refers_to.size == 0 ? nil : chunk.refers_to.entries.sort
  nested_in = chunk.nested_in.size == 0 ? nil : chunk.nested_in.entries.sort
  locations =
    chunk.instances.entries \
         .map { |instance| instance.location } \
         .sort
  lines = chunk.instances.entries[0].content \
       .map { |content|
              content.class == Instance ? nest_chunk(chunk, content) \
                                        : content.escape_xhtml }
  $chunk_template.run(binding)
end

def nest_chunk(from_chunk, to_instance)
  indent = to_instance.indentation || ''
  from = from_chunk.name
  to = to_instance.chunk.name
